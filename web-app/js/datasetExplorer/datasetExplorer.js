		...
		i++) {
			...
			dts.notifyDrop = function (source, e, data) {
				if (source.tree.id == "previousQueriesTree") {
					...
					var childrenIsNumeric = false;
					
					if (data.node.attributes.oktousevalues == "Y" && data.node.attributes.leaf == false){
						childrenIsNumeric = true;
						if (data.node.expanded==false)
							data.node.expand(test){
								
								console.debug("EXPAND FINISHED!");
							});
...
//							
					...
					console.debug("Numeric children == " + childrenIsNumeric);
//					for (var i = 0; i < data.node.childNodes.size(); i++){
//						if (data.node.childNodes[i].attributes.oktousevalues == "Y"){
//							childrenIsNumeric = true;
//							break;
//						}
						//childrenIsNumeric = false;
//					}
					multipleNodes = new Array();
					var firstConcept;
					if (childrenIsNumeric){
						for (var i = 0; i < data.node.childNodes.size(); i++){
							console.debug("going through children " + i);
							concept = createPanelItemNew(this.el, convertNodeToConcept(data.node.childNodes[i]));
							multipleNodes[i] = concept;
							if (i == 0){
								firstConcept = concept;
								console.debug("First item:" + data.node.childNodes[i].attributes.text)
								selectConcept(concept);
								showSetValueDialog();		
							}
							else {
								var valuemode=firstConcept.getAttribute('setvaluemode');
								var highvalue=firstConcept.getAttribute('setvaluehighvalue');
								var lowvalue=firstConcept.getAttribute('setvaluelowvalue');
								var units=firstConcept.getAttribute('setvalueunits');
								var operator=firstConcept.getAttribute('setvalueoperator');
								var valuehighlowselect = firstConcept.getAttribute('setvaluehighlowselect');
//								setValue(concept, "numeric", "LT", "", "", "", "")
								console.debug(valuemode + " " + operator+ " " + valuehighlowselect+ " " + highvalue+ " " + lowvalue+ " " + units);
								setValue(concept, valuemode, operator, valuehighlowselect, highvalue, lowvalue, units);
							}
